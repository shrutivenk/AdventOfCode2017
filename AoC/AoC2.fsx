let input = """3093	749	3469	142	2049	3537	1596	3035	2424	3982	3290	125	249	131	118	3138
...
let checksum =  input.Split([|'\n'|])
                |> Array.map(fun x ->   let values = x.TrimEnd([|'\r'|]).Split([|'\t'|]) 
                                                    |> Array.toSeq
                                                    |> Seq.map(fun x -> x |> int)
